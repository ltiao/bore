...
class BO(nn.Module):
    """
    A simple BO implementation using scikit learn
    ...
    def __init__(self,
        ...
        self.gamma = torch.tensor(gamma)  # ucb parameter trading exploration vs exploitation
        self.refine = refine
    ...
    def step(self):
        ...
        self.regressor.fit(self.X, self.y, nepoch=self.nepoch, batch_size=None)

        # Finds the maximum of the classifier and collects new candidates
        x_new = self.find_maximum_of_ucb(niter=20)
    ...
    def find_maximum_of_ucb(self, niter=20):
        ...
        if isinstance(self.regressor, NN):
            for i in range(niter):
                """
                Gradient-based optimisation
                """
                self.optimiser.zero_grad()
                loss = - self.regressor(self.x_star)
                loss.backward()
                self.optimiser.step()
                if self.constraints is not None:
                    with torch.no_grad():
            ...
            self.x_star = torch.tensor(res.x, dtype=torch.float32).reshape([1, self.dim])

        if self.verbose:
            print(f"BO-UCB: x_t = {self.x_star.detach().numpy()}")
